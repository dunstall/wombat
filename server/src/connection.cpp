...
bool Connection::Read() {
  ...
  if ((n = read(connfd_, buf_.data(), kReadBufSize)) == -1) {
    ...
    const std::optional<record::Request> record = record::Request::Decode(buf_);
    if (record) {
...
std::vector<record::Request> Connection::Received() {
  std::vector<record::Request> recv = std::move(received_);
  received_ = std::vector<record::Request>{};
  return recv;
}
