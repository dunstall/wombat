...
SystemLog::SystemLog(const std::filesystem::path& path,
  ...
  : offsets_{std::make_shared<SystemSegment>(OFFSET_SEGMENT_ID, path, segment_limit)},
    path_{path},
    segments_{},
    active_{1},
    segment_limit_{segment_limit} {
  segments_.emplace(active_, std::make_shared<SystemSegment>(active_, path, segment_limit));
...
void SystemLog::Append(const std::vector<uint8_t>& data) {
  ...
  if (segment->is_full()) {
    ++active_;
    segments_.emplace(active_, std::make_shared<SystemSegment>(active_, path_, segment_limit_));
...
std::shared_ptr<Segment> SystemLog::LookupSegment(uint32_t id) {
  ...
  if (segments_.find(id) == segments_.end()) {
    segments_.emplace(id, std::make_shared<SystemSegment>(id, path_, segment_limit_));
  }
  // Allow at() to throw as should never happen if the id is in offsets.
